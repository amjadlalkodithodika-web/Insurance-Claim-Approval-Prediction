...
with col2:
    ...
    policy_type=st.selectbox("Enter policy Type",poli_options,index=None,
    ...
    past_claim=st.number_input("Past Claim",min_value=0,max_value=15,step=1,value=None,
    placeholder="Past Claims...")
    doc_sbmt=st.number_input("How Many Documment Submitted",min_value=0,max_value=15,step=1,value=None,
    placeholder="Documment's Count...")
...
with col2:
    if st.button("Click"):
        try:
            if policy_type is None or fraud_risk is None:
                st.warning("⚠️ Please Fill the columns before prediction.")
            else:
                # Safe to encode now
